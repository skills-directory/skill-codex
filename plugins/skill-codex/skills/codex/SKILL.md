...
## Running a Task
1. Ask the user (via `AskUserQuestion`) which model to run (`gpt-5.4`, `gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.2`, `gpt-5.1-codex-max`, or `gpt-5.1-codex-mini`) AND which reasoning effort to use (`low`, `medium`, `high`, or `xhigh`) in a **single prompt with two questions**.
